...
var PowerBrowserCore = (() => {
  ...
  __export(index_exports, {
    ...
    QUICK_SWITCHER_RESULT_TYPES: () => QUICK_SWITCHER_RESULT_TYPES,
    ...
    emptyJsonLeafValues: () => emptyJsonLeafValues,
    ...
    isQuickSwitcherResultPreferences: () => isQuickSwitcherResultPreferences,
    ...
    normalizeQuickSwitcherResultPreferences: () => normalizeQuickSwitcherResultPreferences,
    normalizeUrlWithoutQuery: () => normalizeUrlWithoutQuery,
    query: () => query,
    ...
    for (const definition of definitions) {
      ...
      if (definition?.type === "choice") {
        ...
        const values = options.map(
          (option) => option && typeof option === "object" && "value" in option ? option.value : void 0
        );
        if (options.length < 2 || options.some(
          (option) => !option || typeof option !== "object" || !("value" in option) || !("label" in option) || typeof option.label !== "string" || !option.label
      ...
      if (definition?.type === "number") {
        ...
        if (!Number.isFinite(min) || !Number.isFinite(max) || !Number.isInteger(min) || !Number.isInteger(max) || min > max || !Number.isFinite(defaultValue) || !Number.isInteger(defaultValue) || defaultValue < min || defaultValue > max) {
          errors.push(
            `Number setting "${definition.key}" needs a default between its min and max.`
          );
        }
      }
      if (definition?.type === "sortable-list" && (!Array.isArray(definition.defaultValue) || !Array.isArray(definition.items))) {
        errors.push(
  ...
  // src/core/quick-switcher-preferences.ts
  var QUICK_SWITCHER_RESULT_TYPES = Object.freeze([
  ...
  function normalizeQuickSwitcherResultPreferences(value) {
  ...
  function isQuickSwitcherResultPreferences(value) {
    if (!Array.isArray(value) || value.length !== QUICK_SWITCHER_RESULT_TYPES.length) {
      return false;
    }
    const normalized = normalizeQuickSwitcherResultPreferences(value);
    return normalized.every(
      (item, index) => item.id === value[index]?.id && typeof value[index]?.enabled === "boolean"
    );
  }
  function normalizeUrlWithoutQuery(value, base = "https://power-browser.invalid/") {
    try {
  ...
  function emptyJsonLeafValues(value) {
...
(async function () {
  ...
  function applySettingChange(navigator, definition, value) {
    ...
    if (
      definition.key === "runtimeSearchResultTypes" &&
